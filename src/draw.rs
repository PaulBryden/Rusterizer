...
pub fn draw_line(x0_param: i32, y0_param: i32, x1_param: i32, y1_param: i32, framebuffer: &mut Framebuffer, color: u32)
{
    let mut x0 = x0_param;
    let mut y0 = y0_param;
    let mut x1 = x1_param;
...
pub fn draw_textured_triangle(
    ...
    pDepthBuffer: &mut Vec<f32>,
    screen_width: &i64,
) {
    let mut x1 = x1_param.clone();
    let mut y1 = y1_param.clone();
    let mut u1 = u1_param.clone();
    let mut v1 = v1_param.clone();
    let mut w1 = w1_param.clone();
    let mut x2 = x2_param.clone();
    let mut y2 = y2_param.clone();
    let mut u2 = u2_param.clone();
    let mut v2 = v2_param.clone();
    let mut w2 = w2_param.clone();
    let mut x3 = x3_param.clone();
    let mut y3 = y3_param.clone();
    let mut u3 = u3_param.clone();
    let mut v3 = v3_param.clone();
    let mut w3 = w3_param.clone();
    ...
    let mut dv2: f32 = v3 - v1;
    let mut du2: f32 = u3 - u1;
    let mut dw2: f32 = w3 - w1;
    ...
    if (dy1 != 0) {
        for i in y1..(y2) {
            ...
            for j in ax..bx {
                ...
                if (tex_w > pDepthBuffer[(i * screen_width + j) as usize]) {
                    framebuffer.set_color_at(
                        &(j as u32),
                        &(i as u32),
                        *tex.get_color_at_normalized_coord(
                            &((tex_u / tex_w)),
                            &((tex_v / tex_w)),
                        ),
                    );

                    pDepthBuffer[(i * screen_width + j) as usize] = tex_w;
                }
                t += tstep;
            }
        }
    ...
    if (dy1 != 0) {
        for i in y2..(y3) {
            ...
            let mut tstep: f32 = 1.0 / ((bx - ax) as f32);
            let mut t: f32 = 0.0;
