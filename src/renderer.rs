...
use crate::{geometry::{
    matrix_make_identity, matrix_make_projection, matrix_make_rotation_x, matrix_make_rotation_y,
    ...
    vector_add, vector_cross_product, vector_dot_product, vector_sub, Mat4x4, Mesh, Triangle,
    Vec3d, vector_div, vector_mul,
}, draw::draw_textured_triangle, framebuffer::Framebuffer};
...
impl Renderer<'_> {
    pub fn new(
        ...
        Renderer {
            ...
            framebuffer_clear_color: framebuffer_clear_color,
            mat_projection: mat_projection,
            vec_camera: vec_camera,
            vec_look_dir: vec_look_dir,
...
pub fn render(&mut self, time_elapsed: f32, framebuffer: &mut Framebuffer)
{
        ...
        let mut mat_world: Mat4x4 = matrix_make_identity();
        ...
        for mesh in self.meshes.iter() {
            ...
            for tri in mesh.tris.iter() {
                ...
                let mut normal: Vec3d = Vec3d::default();
                let mut line1: Vec3d = Vec3d::default();
                let mut line2: Vec3d = Vec3d::default();

                line1 = vector_sub(&tri_transformed.points[1], &tri_transformed.points[0]);
                line2 = vector_sub(&tri_transformed.points[2], &tri_transformed.points[0]);
                ...
                if (vector_dot_product(&normal, &v_camera_ray) < 0.0) {
                    ...
                    for i in 0..nClippedTriangles {
                        if (i == 0) {
                            // Project triangles
                            tri_projected.points[0] = matrix_multiply_vector(&self.mat_projection, &clipped_1.points[0]);
                            tri_projected.points[1] = matrix_multiply_vector(&self.mat_projection, &clipped_1.points[1]);
                            tri_projected.points[2] = matrix_multiply_vector(&self.mat_projection, &clipped_1.points[2]);
                            ...
                            // Project triangles
                            tri_projected.points[0] = matrix_multiply_vector(&self.mat_projection, &clipped_2.points[0]);
                            tri_projected.points[1] = matrix_multiply_vector(&self.mat_projection, &clipped_2.points[1]);
                            tri_projected.points[2] = matrix_multiply_vector(&self.mat_projection, &clipped_2.points[2]);
                        ...
                        tri_projected.texture_points[0].u = tri_projected.texture_points[0].u / tri_projected.points[0].w;
                        tri_projected.texture_points[1].u = tri_projected.texture_points[1].u / tri_projected.points[1].w;
                        tri_projected.texture_points[2].u = tri_projected.texture_points[2].u / tri_projected.points[2].w;

                        tri_projected.texture_points[0].v = tri_projected.texture_points[0].v / tri_projected.points[0].w;
                        tri_projected.texture_points[1].v = tri_projected.texture_points[1].v / tri_projected.points[1].w;
                        tri_projected.texture_points[2].v = tri_projected.texture_points[2].v / tri_projected.points[2].w;
                        ...
                        //Scale/Normalize
                        tri_projected.points[0] = vector_div(&tri_projected.points[0], tri_projected.points[0].w);
                        tri_projected.points[1] = vector_div(&tri_projected.points[1], tri_projected.points[1].w);
                        tri_projected.points[2] = vector_div(&tri_projected.points[2], tri_projected.points[2].w);
                        ...
                        //offset Vertices
                        let vOffsetView: Vec3d = Vec3d {
                            x: 1.0,
                            y: 1.0,
                            ..Default::default()
                        };
                        tri_projected.points[0] = vector_add(&tri_projected.points[0], &vOffsetView);
                        tri_projected.points[1] = vector_add(&tri_projected.points[1], &vOffsetView);
                        tri_projected.points[2] = vector_add(&tri_projected.points[2], &vOffsetView);
            ...
            for triToRaster in vec_triangles_to_raster {
                let mut clipped_1: Triangle = Triangle::default();
                let mut clipped_2: Triangle = Triangle::default();

                let mut listTriangles: VecDeque<Triangle> = VecDeque::new();

                listTriangles.push_back(triToRaster);
                let mut nNewTriangles: usize = 1;

                for p in 0..4 {
                    let mut nTrisToAdd: u64 = 0;
                    while (nNewTriangles > 0) {

                        // Take triangle from front of list
                        let mut test: Triangle = listTriangles.front().unwrap().clone();
                        ...
                        match (p) {
                            0 => {
                                nTrisToAdd = triangle_clip_against_plane(
                                    &mut Vec3d {
                                        x: 0.0,
                                        y: 0.0,
                                        z: 0.0,
                        ...
                        for w in 0..nTrisToAdd {
                            if (w == 0) {
                                listTriangles.push_back(clipped_1.clone());
                            } else if (w == 1) {
                                listTriangles.push_back(clipped_2.clone());
                            }
                        }
                    }
                    nNewTriangles = listTriangles.len();
                }
